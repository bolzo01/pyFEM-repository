...
Last modified: 2025/12/09 01:29:51
...
class Model:
    ...
    def __init__(self, mesh: Mesh, problem: Problem):
        ...
        self._initial_temperature: np.ndarray | None = None
    ...
    def set_materials(self, materials: dict[str, Material]) -> None:
        self.materials = materials

    # --- Initial temperature API (for heat / diffusion problems) ---

    def set_initial_temperature(self, T0: np.ndarray) -> None:
        """
        Set the initial temperature field for heat-transfer problems.

        Args:
            T0: 1D array of length equal to the total number of DOFs in the
                model's DOF space. For pure HEAT_TRANSFER with scalar
                temperature DOFs, this is typically (num_nodes,).

        Raises:
            ValueError: If the size of T0 does not match the DOF space.
        """
        ...
        if T0.size != ndofs:
            raise ValueError(
                f"Initial temperature vector has size {T0.size}, "
                f"but model DOF space has {ndofs} DOFs."
            )

        self._initial_temperature = T0.copy()

    def has_initial_temperature(self) -> bool:
        """Return True if an initial temperature field has been set."""
        return self._initial_temperature is not None

    def get_initial_temperature(self) -> np.ndarray:
        """
        Return the initial temperature field.

        Returns:
            A 1D numpy array of length equal to the total number of DOFs.

        Raises:
            RuntimeError: If no initial temperature has been set.
        """
        if self._initial_temperature is None:
            raise RuntimeError(
                "Initial temperature has not been set on this model. "
                "Call model.set_initial_temperature(T0) first."
            )
        return self._initial_temperature.copy()
    ...
    def save_vtk(
        ...
        writer = VTKWriter(self, self.solution)
        writer.write(
            fullpath,
