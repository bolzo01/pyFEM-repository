...
Last modified: 2025/12/09 01:35:50
...
def test_incompatible_elements():
    ...
    mesh = pyfem.Mesh(
        ...
        num_elements=2,
        element_connectivity=[[0, 1], [1, 2]],
        element_property_labels=["bar", "triangle"],
    )
...
def test_query_compatibility():
    ...
    compatible = pyfem.get_compatible_problems("triangle")
    print("triangle is compatible with:")
    for physics, dimension in compatible:
        print(f"  - {physics.name}, {dimension.value}D")
    ...
    is_compat = pyfem.is_element_compatible_with_problem("triangle", problem_2d)
    print(f"Is triangle compatible with 2D mechanics? {is_compat}")
