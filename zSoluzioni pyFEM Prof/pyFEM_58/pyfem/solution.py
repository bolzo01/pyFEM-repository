...
Last modified: 2025/12/09 02:33:51
...
class Solution:
    ...
    nodal_displacements : np.ndarray
        Primary nodal field vector for all DOFs.

        - In mechanical problems, this stores displacements.
        - In heat-transfer problems, this stores temperatures (see
          the ``temperature`` convenience property).

    reaction_forces : np.ndarray | None
        Reaction forces at constrained DOFs (typically computed from K u − f).

    element_stresses : list | None
        Per-element stress data (if computed), usually in Voigt notation.

    dof_types : list[DOFType]
        List of global DOF types, defining the structure of the primary field.

    solver_stats : dict[str, int | float]
        Optional dictionary of solver performance metrics and metadata
        (e.g., number of iterations, time step size, CPU time, etc.).
    """

    nodal_displacements: np.ndarray
    reaction_forces: np.ndarray | None = None
    element_stresses: list | None = None
    ...
    def temperature(self) -> np.ndarray:
        """
        Temperature field for heat-transfer problems.

        In diffusion/heat analyses, the primary unknown is the nodal
        temperature, which is stored in ``nodal_displacements`` for
        historical/structural reasons. This property provides a more
        semantically meaningful alias:

            solution.temperature  <==>  solution.nodal_displacements

        Use this in heat examples for clarity.
        """
