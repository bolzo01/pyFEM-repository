...
Last modified: 2025/11/27 14:29:57
...
class DOFConstraintRegistry:
    ...
    def update_dirichlet_value(self, dof: int, value: float) -> None:
        """
        Override an existing prescribed displacement value.

        Used by incremental displacement-controlled steps.
        Does NOT perform conflict checks (unlike set_dirichlet_value).
        """
        # Must already be a Dirichlet DOF
        if self._constraint_type.get(dof) != ConstraintType.DIRICHLET:
            raise ValueError(f"Cannot update DOF {dof}: not a Dirichlet DOF.")
