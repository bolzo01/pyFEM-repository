...
The use of functions and type hints is demonstrated throughout the script.
...
def invert_global_stiffness_matrix(
    ...
    for i in range(n):
        ...
        if abs(diag_factor) < epsilon:
            raise ValueError("Matrix is singular or nearly singular and cannot be inverted.")
        ...
        for k in range(n):
            if k != i:
                ...
                for j in range(n):
                    global_stiffness_matrix[k][j] -= factor * global_stiffness_matrix[i][j]
...
global_stiffness_matrix, global_force_vector = apply_prescribed_displacements(
    prescribed_displacements, global_stiffness_matrix, global_force_vector, total_dofs
)
...
inverse_global_stiffness_matrix = invert_global_stiffness_matrix(global_stiffness_matrix)
...
compute_strain_energy_global(total_dofs, original_global_stiffness_matrix, nodal_displacements)
