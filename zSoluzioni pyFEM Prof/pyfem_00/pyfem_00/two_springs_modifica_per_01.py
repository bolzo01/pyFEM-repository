...
def assemble_global_stiffness_matrix(
    ...
    """
    Assembla la matrice di rigidezza globale a partire dalle matrici locali
    degli elementi in un modello a una dimensione (ad esempio, molle o barre).

    Descrizione del processo:
    - Per ciascun elemento nel modello, viene generata la matrice di rigidezza locale
      (2x2) basata sul valore di rigidezza dell’elemento.
    - I gradi di libertà locali dell’elemento vengono mappati nei corrispondenti gradi
      di libertà globali attraverso la matrice di connettività.
    - I contributi delle matrici locali vengono poi sommati nella matrice di rigidezza
      globale, nel rispetto della connettività del sistema.

    Parametri
    ----------
    num_elements : int
        Numero totale di elementi nel modello.
    element_stiffness : list[float]
        Lista contenente la rigidezza (k) di ciascun elemento.
    element_connectivity : list[list[int]]
        Matrice di connettività che associa ciascun elemento ai nodi globali.
        Ogni riga definisce i due nodi estremi dell’elemento.
    global_stiffness_matrix : list[list[float]]
        Matrice di rigidezza globale (inizializzata a zeri) in cui verranno
        sommati i contributi locali.
    Output
    ------
    La funzione aggiorna direttamente `global_stiffness_matrix` inserendo
    i contributi di ciascun elemento. Non restituisce un valore esplicito.
...
def creation_of_forces_vector(applied_forces, global_force_vector):
    """
    Assegna le forze esterne ai rispettivi gradi di libertà (DOF)
    nel vettore globale delle forze.

    Parametri
    ----------
    applied_forces : list[list[float]]
        Lista di coppie [dof, valore] che rappresentano le forze applicate.
    global_force_vector : list[float]
        Vettore globale delle forze, inizializzato a zeri.

    Ritorna
    -------
    list[float]
        Il vettore globale delle forze aggiornato.
    """
...
def assemble_constrain_displacement(
    total_dofs, global_stiffness_matrix, global_force_vector, prescribed_displacements
):
    """
    Applica i vincoli di spostamento (Dirichlet) alla matrice di rigidezza e al vettore delle forze globali.

    Parametri:
        total_dofs (int): numero totale di gradi di libertà del sistema
        global_stiffness_matrix (2D list o np.ndarray): matrice di rigidezza globale
        global_force_vector (list o np.ndarray): vettore dei carichi globali
        prescribed_displacements (list di tuple): [(dof, valore_imposto), ...]

    Ritorna:
        tuple: (global_stiffness_matrix, global_force_vector) aggiornati
    """
    ...
    return (global_force_vector, global_stiffness_matrix)


def inverse_of_global_stiffness_matrix(global_stiffness_matrix):
    """
    Calcola l'inversa della matrice di rigidezza globale tramite il metodo di eliminazione di Gauss-Jordan.

    Parametri:
        global_stiffness_matrix (list[list[float]]): matrice di rigidezza globale quadrata.

    Ritorna:
        list[list[float]]: matrice inversa della matrice di rigidezza globale.

    Solleva:
        ValueError: se la matrice è singolare o quasi singolare (non invertibile).

    Note:
        - La funzione modifica la matrice originale in-place.
        - Complessità computazionale: O(n^3).
        - Metodo puramente didattico; per uso numerico reale è preferibile usare NumPy o SciPy.
    """
...
for _ in range(total_dofs):
    global_stiffness_matrix.append([0.0] * total_dofs)

# -- Initialize the global force vector with zeros
global_force_vector = [0.0] * total_dofs
...
global_force_vector = creation_of_forces_vector(applied_forces, global_force_vector)


# - Boundary conditions: Constrain
global_stiffness_matrix, global_force_vector = assemble_constrain_displacement(
    total_dofs, global_stiffness_matrix, global_force_vector, prescribed_displacements
)
...
inverse_global_stiffness_matrix = inverse_of_global_stiffness_matrix(
    global_stiffness_matrix
)
...
for i in range(total_dofs):
    ...
    for j in range(total_dofs):
        displacement += inverse_global_stiffness_matrix[i][j] * global_force_vector[j]
...
# - Compute strain energy by looping on elements
...
# - Compute strain energy from global solution (U = 0.5 * u^T * K * u)
total_strain_energy = 0.0
