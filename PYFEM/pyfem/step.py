...
from .solvers import DiffusionExplicitSolver, LinearStaticSolver
...
class Step:
    ...
    verbose: bool = True
    total_time: float = 1.0  # total "simulation time" for transient problems
    ...
    def execute(
        ...
        elif self.procedure == ProcedureType.DIFFUSION_EXPLICIT:
            # Safety: explicit diffusion is only meaningful for HEAT_TRANSFER problems.
            if model.problem.physics is not Physics.HEAT_TRANSFER:
                raise ValueError(
                    f"Step '{self.name}' is configured with procedure "
                    f"DIFFUSION_EXPLICIT, but the model physics is "
                    f"{model.problem.physics.name}. "
                    "Explicit diffusion is currently supported only for "
                    "Physics.HEAT_TRANSFER. Please check your Problem "
                    "definition."
                )

            solution = self._execute_diffusion_explicit(model, model_state, use_sparse)
    ...
    def _execute_static_linear_incremental(
        self,
        model: Model,
        model_state: ModelState,
        use_sparse: bool = True,
    ...
    def _write_incremental_output(self, model, solution, inc):
        ...
        write_displacements = "displacements" in fields
        write_stresses = "element_stresses" in fields
        write_temperature = "temperature" in fields
        ...
        model.save_vtk(
            ...
            stresses=write_stresses,
            temperatures=write_temperature,
    ...
    def _execute_diffusion_explicit(
        self,
        model: Model,
        model_state: ModelState,
        use_sparse: bool = True,  # currently unused, kept for API symmetry
    ) -> Solution:
        """
        Explicit forward Euler time integration for diffusion / heat equation.

        Uses:
            M_lumped dT/dt + K T = F
        ...
        t0 = 0.0
        t1 = self.total_time
        n_steps = self.increments  # reuse 'increments' as number of time steps

        if n_steps <= 0:
            raise ValueError("Step.increments must be >= 1 for DIFFUSION_EXPLICIT")

        dt = (t1 - t0) / n_steps

        if self.verbose:
            print(f"[{self.name}] Diffusion explicit: t0={t0}, t1={t1}, dt={dt}")

        # 1) Assemble system matrices and load vector once
        solver = DiffusionExplicitSolver(model)
        solver.assemble_mass_and_stiffness()  # fills M_lumped and K
        solver.assemble_force_vector()  # fills F (may be time-dependent)

        # CENTRAL stability check
        if not solver.check_stability(dt, verbose=self.verbose):
            dt_crit = solver.compute_critical_timestep()
            raise ValueError(
                f"Unstable explicit time step in step '{self.name}'.\n"
                f"  dt      = {dt:.6e} s\n"
                f"  dt_crit = {dt_crit:.6e} s\n"
                "Please reduce total_time and/or increase increments."
            )

        # 2) Initial condition T^0
        if model_state.step_count == 0:
            T_n = solver.initial_temperature()  # zeros or user-defined
        else:
            # Reuse previous step solution (interpret DOFs as temperature)
            T_n = model_state.current_solution.nodal_displacements.copy()
        ...
        for i in range(1, n_steps + 1):
            ...
            # Enforce Dirichlet BCs at this time level
            solver.apply_dirichlet_temperatures(T_np1, time)

            # Wrap into Solution (interpret as temperature field)
            last_solution = Solution(
                nodal_displacements=T_np1,
                reaction_forces=None,  # not meaningful for diffusion
                dof_types=model.dof_space.global_dof_types,
                solver_stats={
                    "time": time,
                    "dt": dt,
                },
            ...
            # Update model with the current solution
            model.set_solution(last_solution)
            T_n = T_np1

            # --- NEW: VTK output every output_frequency steps ---
            if (
                self.output_file is not None
                and self.output_frequency > 0
                and (i % self.output_frequency == 0)
        ...
        if last_solution is None:
            raise RuntimeError("No solution computed in explicit diffusion step.")
