...
Last modified: 2025/11/17 23:15:29
...
class Bar1D(FiniteElement):
    ...
    Supports:
      ...
      - Numerical stiffness and stress via Gauss integration
    """
    ...
    def compute_stiffness(self, material, x_nodes):
        ...
        for xi, w in quad:
            ...
            J = self.jacobian(x_nodes, xi)
            D = material.constitutive_matrix  # 1x1 matrix
    ...
    def compute_strain_at_gauss_points(self, x_nodes, u_nodes):
        """Return strain values at gauss points (1 value per GP)."""
        if self.integration == "analytical":
            # closed form
            L = x_nodes[1] - x_nodes[0]
            return np.array([(u_nodes[1] - u_nodes[0]) / L])

        # Numerical version
        strains = []
        for xi, w in self.integration_points():
            B = self.B_matrix(x_nodes, xi)
            eps = B @ u_nodes  # scalar
            strains.append(float(eps))
        return np.array(strains)

    def compute_stress_at_gauss_points(self, material, x_nodes, u_nodes):
        eps = self.compute_strain_at_gauss_points(x_nodes, u_nodes)
    ...
    def compute_stress(self, material, x_nodes, u_nodes):
        ...
        Return stress values:
          - analytical -> 1 value at midpoint
          - numerical -> one value per Gauss point
        """
        return self.compute_stress_at_gauss_points(material, x_nodes, u_nodes)
