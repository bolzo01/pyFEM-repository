...
Last modified: 2025/11/10 09:59:53
...
def _compute_bar_1d_numerical_physical(
    ...
    for ip in range(n_gauss):
        ...
        # Map quadrature point from reference [-1,1] to physical [x1,x2]
        x_gp = 0.5 * (1 + xi_ip) * L + x1  # physical coordinate of quadrature point
...
def bar1d_linear_B_matrix(x_nodes: np.ndarray, x_gp: float) -> tuple[np.ndarray, float]:
    ...
    Args:
        x_nodes: array([x1, x2])
        x_gp: quadratuee point location in physical space
